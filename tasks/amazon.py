...
class AmazonTask(BaseTask):
    def __init__(self, split):
        data_file = f"{DATA_DIR}/amazon/{split}.json"
    ...
    def get_prompt(self):
        with open(f"{DATA_DIR}/../prompts/cot_amazon.txt", "r") as fin:
            prompt = fin.read() 
        return prompt
    
    def get_userid(self, idx):
        return self.data[idx]['userid_encoded']
    
    def get_history_actions(self, idx):
        return self.data[idx]["pos_seq_name"]