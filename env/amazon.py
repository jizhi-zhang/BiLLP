...
class Amazon_Grounding_Model(object):
    
    def __init__(self, model_path, config, batch_size=16):
        super(Amazon_Grounding_Model, self).__init__()
        ...
        if 'llama' in self.model_path:
            ...
            self.amazon_embedding = torch.load("./env/amazon/amazon_embedding_task.pt")['embeddings'].cuda()
            self.amazon_index = torch.load("./env/amazon/amazon_embedding_task.pt")['indexs'].cuda()
            
            with open('./env/amazon/datamaps.json') as f:
    ...
    def get_top1_near_item(self, item_list):
        ...
        dist = torch.cdist(item_embeddings, self.amazon_embedding, p=2)
        
        indices = torch.argmin(dist, dim=1)
        
        amazon_id = self.amazon_index[indices].detach().cpu().numpy()
        
        amazon_item = [self.id2item[str(int(id))] for id in amazon_id]
        return amazon_item
    
    def get_topk_near_item(self, item_list, k):
        item_embeddings = []
        task_list = [f'The type of {item} book is' for item in item_list]
        ...
        amazon_item = [[self.id2item[str(int(id))] for id in id_list] for id_list in amazon_id]
        return amazon_item