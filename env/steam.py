...
class SteamENV(object):
    
    def __init__(self, config, split) -> None:
        path = os.path.join(config.env_path, 'steam/')
        
        self.item2id = get_item2id(os.path.join(path, 'datamaps.json'))
        self.id2item = get_id2item(os.path.join(path, 'datamaps.json'))
        self.reward_mat = load_mat(os.path.join(path, f'steam_{split}.npy'))
...
class Steam_Grounding_Model(object):
    
    def __init__(self, model_path, config, batch_size=16):
        super(Steam_Grounding_Model, self).__init__()
        ...
        if 'llama' in self.model_path:
            ...
            self.steam_embedding = torch.load("./env/steam/steam_embedding_task.pt")['embeddings'].cuda()
            self.steam_index = torch.load("./env/steam/steam_embedding_task.pt")['indexs'].cuda()
            
            with open('./env/steam/datamaps.json') as f:
    ...
    def get_top1_near_item(self, item_list):
        ...
        dist = torch.cdist(item_embeddings, self.steam_embedding, p=2)
        
        indices = torch.argmin(dist, dim=1)
        
        steam_id = self.steam_index[indices].detach().cpu().numpy()
        
        steam_item = [self.id2item[str(int(id))] for id in steam_id]
        return steam_item
    
    def get_topk_near_item(self, item_list, k):
        item_embeddings = []
        task_list = [f'The type of {item} is' for item in item_list]
        ...
        steam_item = [[self.id2item[str(int(id))] for id in id_list] for id_list in steam_id]
        return steam_item